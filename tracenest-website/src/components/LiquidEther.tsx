...
`;
    const color_frag = `
		...
		float normalizedVelocity = clamp(lenv, 0.0, 1.0);
		float palettePosition = pow(normalizedVelocity, 0.68);
		vec3 c = texture2D(palette, vec2(palettePosition, 0.5)).rgb;
		float coreFactor = smoothstep(0.62, 1.0, normalizedVelocity);
		float outA = lenv * maxAlpha * mix(0.78, 1.12, coreFactor);
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        this.props = props;
        this.lastUserInteraction = performance.now() - Math.max(0, props.autoResumeDelay || 0);
  ...
  useEffect(() => {
    ...
    const outputUniforms = (webgl.output as { material?: { uniforms?: Record<string, { value: unknown }> } })?.material
      ?.uniforms;
    if (outputUniforms?.maxAlpha) outputUniforms.maxAlpha.value = maxAlpha;
    if (resolution !== prevRes) sim.resize();
  }, [
    mouseForce,
    cursorSize,
